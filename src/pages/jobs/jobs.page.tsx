  ...
  return (
    <AppShell fullWidth>
      <div className="overflow-hidden rounded-xl border border-[#e8eaf0] bg-white">
        <section className="border-b border-[#dce5f5] bg-[#eef3fb] p-5 pb-4">
          <h2 className="text-[22px] font-bold text-[#0f172a]">AI Job Match</h2>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <div className="relative flex min-w-[340px] flex-1 items-center">
              <button
                ...
                className="flex h-10 items-center gap-1 rounded-l-lg border border-r-0 border-[#d0d8f0] bg-white px-3 text-xs font-medium text-[#0f172a] hover:border-[#0041c8]"
              >
                {searchMode === "resume" ? (
                  <FileText className="h-3.5 w-3.5 text-[#4a5068]" />
                ) : (
                  <Search className="h-3.5 w-3.5 text-[#4a5068]" />
                )}
                <span>{searchMode === "resume" ? "Resume" : "Keywords"}</span>
                <ChevronDown className="h-3.5 w-3.5 text-[#8892a4]" />
              </button>
              <input
                value={keyword}
                ...
                className="h-10 flex-1 rounded-r-lg border border-[#d0d8f0] px-3 text-xs text-[#4a5068] outline-none focus:border-[#0041c8]"
              />

              {openDropdown === "mode" && (
                <div className="absolute left-0 top-[44px] z-20 w-44 rounded-lg border border-[#d0d8f0] bg-white py-1 shadow-lg">
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-[#1a1a2e] hover:bg-[#f0f3ff]"
                  >
                    <FileText className="h-3.5 w-3.5 text-[#8892a4]" />
                    Use my resume
                  </button>
                  <button
                    type="button"
                    ...
                    className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-[#1a1a2e] hover:bg-[#f0f3ff]"
                  >
                    <Search className="h-3.5 w-3.5 text-[#8892a4]" />
                    Search by keyword
                  </button>
                </div>
              )}
            ...
            <div className="relative w-[220px]">
              <MapPin className="pointer-events-none absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-[#8892a4]" />
              <input
                value={location}
                onChange={(e) => setLocation(e.target.value)}
                placeholder="Your current location"
                className="h-10 w-full rounded-lg border border-[#d0d8f0] py-2 pl-9 pr-3 text-xs outline-none focus:border-[#0041c8]"
              />
            </div>

            <button
              type="button"
              className="h-10 rounded-lg bg-[#0041c8] px-5 text-sm font-semibold text-white hover:bg-[#3366d6]"
            >
              Search
            </button>
          </div>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1 rounded-md border border-[#d0d8f0] bg-white px-3 py-1.5 text-xs text-[#4a5068] hover:border-[#0041c8] hover:text-[#0041c8]"
              >
                Date range
                <ChevronDown className="h-3.5 w-3.5" />
              </button>
              {openDropdown === "date" && (
                <div className="absolute left-0 top-[34px] z-20 w-36 rounded-lg border border-[#d0d8f0] bg-white py-1 shadow-lg">
                  {["Last 3 days", "Last week", "Last month", "Any time"].map(
                    (item) => (
                      <button
                        ...
                        className="block w-full px-3 py-2 text-left text-sm hover:bg-[#f0f3ff]"
                      >
                        {item}
                      </button>
                    ),
            ...
            <div className="relative">
              ...
              {openDropdown === "remote" && (
                <div className="absolute left-0 top-[34px] z-20 w-32 rounded-lg border border-[#d0d8f0] bg-white py-1 shadow-lg">
            ...
            <button
              ...
              className="text-xs text-[#4a5068] hover:text-[#0041c8]"
            >
              Clear all
            </button>
          </div>
        ...
        <section className="bg-[#f5f6fa] p-5">
          <div className="mb-4 flex items-start gap-2 text-sm leading-relaxed text-[#2b4a7f]">
            <Info className="mt-0.5 h-4 w-4 shrink-0 text-[#1f6fcf]" />
            <p>
          ...
          >
            <aside
              className="overflow-hidden rounded-xl border border-[#e0e4ee] bg-white flex flex-col"
              style={
                leftPanelHeight ? { height: `${leftPanelHeight}px` } : undefined
              }
            >
              <div className="relative border-b border-[#edf0f5] px-4 py-3">
                <button
                  type="button"
                ...
                {openDropdown === "sort" && (
                  <div className="absolute left-4 top-12 z-20 w-[200px] overflow-hidden rounded-lg border border-[#e5e7eb] bg-white shadow-lg">
                    <div className="border-b border-[#f0f1f5] px-4 py-2.5 text-sm text-[#6b7280]">
                      Sort by
                    </div>
                    <button
                      type="button"
                      ...
                      className={`flex w-full items-center justify-between px-4 py-2.5 text-left text-sm text-[#374151] hover:bg-[#eceef2] ${
                        sortBy === "relevance" ? "bg-[#f3f4f6]" : "bg-white"
                      }`}
                    >
                      Relevance
                    ...
                    <button
                      ...
                      className={`flex w-full items-center justify-between px-4 py-2.5 text-left text-sm text-[#374151] hover:bg-[#eceef2] ${
                        sortBy === "date" ? "bg-[#f3f4f6]" : "bg-white"
                      }`}
                    >
                      Date
                  ...
                  return (
                    <button
                      ...
                      className={`mb-2 w-full rounded-xl border p-4 text-left transition-colors ${
                        isActive
                          ? "border-[#d0dbf8] bg-[#f0f4ff]"
                          : "border-[#ebeff5] bg-white hover:bg-[#f8f9ff]"
                      }`}
                    >
                      {job.goodMatch && (
                        <span className="mb-1 inline-flex items-center gap-1 rounded-full bg-[#e6f9f0] px-2 py-1 text-[11px] font-semibold text-[#15803d]">
                      ...
                      <div
                        className={`text-sm font-semibold ${
                          isActive ? "text-[#0041c8]" : "text-[#0f172a]"
                        }`}
                      >
                        {job.title}
                      </div>
                      <div className="mt-0.5 text-xs font-medium text-[#0f172a]">
                        {job.company}
                      </div>
                      <div className="mt-0.5 text-xs text-[#8892a4]">
                        {job.location}
                      </div>
                      <div className="mt-1 text-[11px] text-[#b0b8cc]">
                        {job.postedAt}
                      </div>
                    </button>
                  );
            ...
            <article
              ref={rightPanelRef}
              className="rounded-xl border border-[#e0e4ee] bg-white"
            >
              <div className="border-b border-[#f0f1f5] px-6 pb-4 pt-5">
                ...
                <div className="mt-3 space-y-1.5 text-sm text-[#4a5068]">
                  <div className="flex items-center gap-2">
                    <MapPin className="h-3.5 w-3.5 text-[#8892a4]" />
                    {selectedJob.location}
                  </div>
                  <div className="flex items-center gap-2">
                    <BriefcaseBusiness className="h-3.5 w-3.5 text-[#8892a4]" />
                    {selectedJob.type}
                  </div>
                </div>
                ...
                <div className="mt-4 flex items-center gap-2">
                  <button
                    type="button"
                    className="rounded-lg bg-[#0041c8] px-4 py-2 text-sm font-semibold text-white hover:bg-[#3366d6]"
                  >
                    Scan
                  </button>
                  <button
                    type="button"
                    className="inline-flex items-center gap-1 rounded-lg border border-[#d0d8f0] bg-white px-3 py-2 text-sm text-[#4a5068] hover:border-[#0041c8] hover:text-[#0041c8]"
                  >
              ...
              <div className="px-6 py-5 text-sm leading-relaxed text-[#2d3748]">
                <p className="mb-3 font-medium text-[#0f172a]">
                  Job Description
                </p>
                ...
                <ul className="mb-4 space-y-1.5">
                  {selectedJob.responsibilities.map((item) => (
                    <li key={item} className="flex items-start gap-2">
                      <span className="mt-1 text-[#0041c8]">•</span>
                      <span>{item}</span>
                    </li>
                  ))}
                </ul>
